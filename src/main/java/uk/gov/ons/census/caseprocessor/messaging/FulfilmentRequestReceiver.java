...
public class FulfilmentRequestReceiver {
  ...
  public FulfilmentRequestReceiver(
      FulfilmentRequestService fulfilmentRequestService, EventLogger eventLogger, CaseService caseService) {
  ...
  public void receiveMessage(Message<byte[]> message) {
    ...
    List<String> smsIndividualPackCodes = List.of("UACIT1", "UACIT2", "UACIT2W", "UACIT3", "UACIT4");
    List<String> printIndividualPackCodes = List.of("P_OR_I1", "P_OR_I2", "P_OR_I2W", "P_OR_IACR3");

    EventDTO event = convertJsonBytesToEvent(message.getPayload());
    Case caze = null, parentCase = null;
    UUID caseId = null;
    ...
    // TODO:CN-174 When packCode for SMS/Paper is in given list (UACIT1, UACIT2, UACIT2W, UACIT3, UACIT4, P_OR_I1, P_OR_I2, P_OR_I2W, P_OR_IACR3)
    // need to create  a Child Case, copy all the case sample data from the main HH case, exclude activity flags receiptReceived or
 ...
 ///
    ...
    if (exportFileTemplate.isPresent() && smsTemplate.isEmpty()) {
        ...
        if (printIndividualPackCodes.contains(fulfilmentRequest.getFulfilmentCode())) {
            ...
            caze = fulfilmentRequestService.processFulfilmentForIndividual(event, smsTemplate, exportFileTemplate);
            if (caze != null) {
                eventLogger.logCaseEvent(caze, "New case created", EventType.NEW_CASE, event, message);
                caseId = caze.getId();
            }
        }
            //TODO: should rebuild the HeaderDTO or just change the case Id and use the same header for child case fulfilment.
